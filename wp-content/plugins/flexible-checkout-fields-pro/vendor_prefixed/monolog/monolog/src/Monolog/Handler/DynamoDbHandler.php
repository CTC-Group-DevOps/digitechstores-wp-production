...
use FCFProVendor\Aws\Sdk;
use FCFProVendor\Aws\DynamoDb\DynamoDbClient;
use FCFProVendor\Monolog\Formatter\FormatterInterface;
use FCFProVendor\Aws\DynamoDb\Marshaler;
use FCFProVendor\Monolog\Formatter\ScalarFormatter;
...
class DynamoDbHandler extends AbstractProcessingHandler
{
    ...
    public function __construct(DynamoDbClient $client, string $table, $level = Logger::DEBUG, bool $bubble = \true)
    {
        ...
        if (defined('FCFProVendor\Aws\Sdk::VERSION') && version_compare(Sdk::VERSION, '3.0', '>=')) {
            $this->version = 3;
