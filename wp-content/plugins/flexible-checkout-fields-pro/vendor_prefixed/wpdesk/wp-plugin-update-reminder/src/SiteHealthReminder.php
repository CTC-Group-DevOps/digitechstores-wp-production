...
class SiteHealthReminder implements Reminder, Hookable
{
    ...
    {
        $results = array('label' => __('You should update plugin', 'flexible-checkout-fields-pro'), 'status' => $status, 'badge' => array('label' => __('Compatibility', 'flexible-checkout-fields-pro'), 'color' => $color), 'description' => $description, 'actions' => '', 'test' => basename($this->reminder_data->get_plugin_dir()));
        return $results;
    }
}
