...
class PluginUpgrade
{
    ...
    {
        global $wp_version;
        $params = ['site_url' => defined('FCFProVendor\WP_HOME') ? \FCFProVendor\WP_HOME : get_home_url(), 'plugin_name' => $this->plugin_info->get_plugin_name(), 'plugin_slug' => $this->plugin_info->get_plugin_slug(), 'plugin_version' => $this->plugin_info->get_version(), 'php_version' => \PHP_VERSION, 'wc_version' => class_exists('WooCommerce') ? \WooCommerce::instance()->version : '', 'wp_version' => $wp_version, 'locale' => str_replace('-', '_', get_bloginfo('language'))];
    ...
    private function react_to_wp_error_response(\WP_Error $remote_response): \stdClass
    {
        ...
        $response->message = wp_kses_post('<span style="color: red" class="error">' . sprintf(esc_html__('Error while connecting to remote server %s. Please contact your hosting provider or try again later. Errors:  ', 'flexible-checkout-fields-pro'), $this->server) . implode(', ', $remote_response->get_error_messages()) . '</span>');
    ...
    private function render_changelog(string $plugin_version, string $changelog): string
    {
        ...
        if (iterator_count($changes) > 0) {
            $changelogFormatter = new \FCFProVendor\WPDesk\License\Changelog\Formatter($changes);
    ...
    private function react_to_nonsense_response($remote_response): \stdClass
    {
        ...
        if (isset($remote_response['response']['code'])) {
            if (in_array($remote_response['response']['code'], [503, 502], \true)) {
                $message = esc_html__('License server is currently unavailable. Please, try again later.', 'flexible-checkout-fields-pro');
            } else {
                $message = sprintf(esc_html__('Error while connecting to remote server %1$s. Please contact with your hosting provider. Cannot parse response. Response code: %2$s Message: %3$s', 'flexible-checkout-fields-pro'), $this->server, $remote_response['response']['code'], $remote_response['body']);
            }
        } else {
            $message = sprintf(esc_html__('Error while connecting to remote server %1$s. Please contact with your hosting provider. Cannot parse response: %2$s', 'flexible-checkout-fields-pro'), $this->server, json_encode($remote_response));
        }
        $response->new_version = '';
