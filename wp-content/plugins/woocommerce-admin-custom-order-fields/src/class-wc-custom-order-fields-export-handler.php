...
class WC_Admin_Custom_Order_Fields_Export_Handler {
	...
	/**
	 * Adds support for Customer/Order CSV Export by adding a column header for each registered admin order field.
	 *
	 * @internal
	 *
	 * @since 1.2.0
	...
	*/
	...
	public function add_fields_to_xml_export_data( $order_data, $order, $generator = null ) {
		...
		if ( $generator && 'custom' === $generator->export_format ) {
			...
			$custom_fields_key = $format_definition['fields']['AdminCustomOrderFields'] ?? null;
